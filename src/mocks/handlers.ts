...
import { balanceSheet, filings, insiderTrades, lateFilings, operations, trendingStocks } from './data'
import { rest } from 'msw'

export const handlers = [
    rest.get(`${API_URL}/app/analytics`, (req, res, ctx) => {
        return res(
            ctx.json(trendingStocks),
        )
    }),
    rest.get(`${API_URL}/company/late-filings`, (req, res, ctx) => {
        return res(
            ctx.json(lateFilings),
        )
    }),
